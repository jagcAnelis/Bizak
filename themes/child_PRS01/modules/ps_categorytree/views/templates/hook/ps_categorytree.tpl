  ...
  {strip}
    {if $nodes|count}
      ...
      <ul class="category-sub-menu">
        {foreach from=$nodes|sortAgeCategoryByAge item=node}

              <li data-depth="{$depth}">
                {if $depth===0}

                  {if isset($smarty.get.id_category) && $node.id == $smarty.get.id_category}
                      <a href="{$node.link}" class="active-category">{$node.name}</a>
                  {else}
