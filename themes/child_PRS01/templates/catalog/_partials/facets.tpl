    ...
    *}
      ...
      {if $facet.displayed}
        <section class="facet clearfix">
          ...
          {foreach from=$facet.filters item="filter"}
            {if $filter.active}{assign var=_collapse value=true}{/if}
          {/foreach}
          ...
          {if $facet.widgetType !== 'dropdown'}
            ...
            {block name='facet_item_other'}
              <ul id="facet_{$_expand_id}" class="collapse{if !$_collapse} in{/if}">
                {foreach from=$facet.filters|sortAgeFiltersByAge key=filter_key item="filter"}
                  {if $filter.displayed}
                    <li>
              ...
              <ul id="facet_{$_expand_id}" class="collapse{if !$_collapse} in{/if}">
                <li>
                  <div class="col-sm-12 col-xs-12 col-md-12 facet-dropdown dropdown">
                    ...
                    <div class="dropdown-menu faced-filter">
                      {foreach from=$facet.filters item="filter"}
                        {if !$filter.active}
                          <a
                            rel="nofollow"
    ...
    {block name='facets_clearall_button'}
          <div id="_desktop_search_filters_clear_all" class="hidden-sm-down clear-all-wrapper">
              <button data-search-url="{$clear_all_link}" class="btn btn-tertiary js-search-filters-clear-all button-faced-filter">
