...
{
    const WK_DONATION_PRICE_TYPE_FIXED = 1;
    const WK_DONATION_PRICE_TYPE_CUSTOMER = 2;
    ...
    public static $definition = array(
        ...
        'fields' => array(
            ...
            'price_type' => array('type' => self::TYPE_INT, 'validate' => 'isUnsignedInt', 'required' => true),
            'price' => array('type' => self::TYPE_FLOAT, 'validate' => 'isPrice', 'required' => true),
            'product_visibility' => array('type' => self::TYPE_INT, 'validate' => 'isUnsignedInt', 'required' => true),
            'show_at_checkout' => array('type' => self::TYPE_INT, 'validate' => 'isUnsignedInt', 'required' => true),
            'advertise' => array('type' => self::TYPE_INT, 'validate' => 'isUnsignedInt', 'required' => true),
            'expiry_date' => array('type' => self::TYPE_DATE, 'validate' => 'isDateFormat'),
            'show_donate_button' => array('type' => self::TYPE_INT, 'validate' => 'isUnsignedInt'),
            'adv_title_color' => array('type' => self::TYPE_STRING, 'validate' => 'isColor'),
            'adv_desc_color' => array('type' => self::TYPE_STRING, 'validate' => 'isColor'),
            'button_text_color' => array('type' => self::TYPE_STRING, 'validate' => 'isColor'),
            'button_border_color' => array('type' => self::TYPE_STRING, 'validate' => 'isColor'),
            'is_global' => array('type' => self::TYPE_INT, 'validate' => 'isUnsignedInt'),
            ...
            'advertisement_description' => array(
                'type' => self::TYPE_HTML,
                'lang' => true,
                'validate' => 'isCleanHtml',
            ),
    ...
    public function getCheckoutDonations($idShop)
    {
        return Db::getInstance()->executeS(
            'SELECT `id_donation_info` FROM `' . _DB_PREFIX_ . 'wk_donation_info` di
    ...
    public function getIdGlobalDonation()
    {
        return Db::getInstance()->getValue(
            'SELECT `id_donation_info` FROM `' . _DB_PREFIX_ . 'wk_donation_info` di WHERE di.is_global = 1'
        );
    }
    ...
    public function updateDonationExpiry()
    {
        if ($idAllDonations = Db::getInstance()->executeS(
            ...
            foreach ($idAllDonations as $idDonationInfo) {
                if (Validate::isLoadedObject($objDonationInfo = new self((int) $idDonationInfo['id_donation_info']))) {
    ...
    public function getTotalDonationCount($idDonationInfo = null)
    {
        if ($idDonationInfo) {
            return Db::getInstance()->getValue(
                'SELECT COUNT(DISTINCT ds.`id_order`) FROM `'. _DB_PREFIX_ .'wk_donation_stats` ds
    ...
    public function getTotalCustomerCount($idDonationInfo = null)
    {
        if ($idDonationInfo) {
            return Db::getInstance()->getValue(
                'SELECT COUNT(DISTINCT ds.`id_customer`) FROM `'. _DB_PREFIX_ .'wk_donation_stats` ds
                WHERE `id_donation_info` ='. (int)$idDonationInfo
            );
        } else {
            return Db::getInstance()->getValue(
    ...
    public function getTotalDonationAmount($idDonationInfo = null)
    {
        if ($idDonationInfo) {
            $total = Db::getInstance()->executeS(
                'SELECT od.`id_currency` , ord.`total_price_tax_incl`
