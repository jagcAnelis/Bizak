...
class an_productattributes extends Module
{
	...
	{
		/*
		Если буду реализовывать - надо проверку существования метода еще делать
	 ...
	 public function getImages($product)
	 {
		...
		$productImages = $imageRetriever->getProductImages($product, $this->context->language);
		
		$images = $this->filterImagesForCombination($productImages, $product['id_product_attribute']);
		
		$coverImage = $imageRetriever->getImage($productObj, $images['0']['id_image']);
