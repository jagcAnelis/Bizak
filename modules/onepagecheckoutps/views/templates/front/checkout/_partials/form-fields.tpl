...
{if $field.type eq 'text' or $field.type eq 'password' or $field.type eq 'email'}
    <input
        ...
        data-validation="{strip}
            {$field.availableValues.validation|escape:'htmlall':'UTF-8'}
            {if $field.maxLength neq 0}
                ,length
            {/if}
            {if
                ($field.name eq 'email' and (isset($isRequestConfirmationEmail) and $isRequestConfirmationEmail)) or
                $field.name eq 'password' and !$isCustomer
            }
                ,confirmation
            {/if}
        {/strip}"
