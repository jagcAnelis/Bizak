...
class OnePageCheckoutPS extends Module
{
    ...
    public function install()
    {
        ...
        if (!parent::install() || !$this->getService('onepagecheckoutps.install.installer')->install()) {
            return false;
        }
    ...
    public function uninstall()
    {
        ...
        if (!$this->getService('onepagecheckoutps.install.installer')->uninstall() ||
            !$this->core->uninstall() ||
            !parent::uninstall()
        ) {
            return false;
    ...
    protected function displayForm()
    {
        ...
        $this->params_back = array(
            ...
            'CRON_LINK'                            => $this->context->link->getModuleLink(
                $this->name,
                'cron',
                array('token' => Tools::encrypt($this->name.'/index'))
            ),
    ...
    private function getCheckoutBetaForm()
    {
        ...
        $form = array(
            'tab' => 'checkoutBetaGeneral',
            'method' => 'post',
            'actions' => array(
                'save' => array(
                    'label' => $this->l('Save'),
                    'class' => 'save-checkoutBeta',
                    'icon'  => 'save',
                ),
            ),
            'options' => $options,
    ...
    private function getHelperTabs()
    {
        ...
        if (version_compare(_PS_VERSION_, '1.7.6', '>=')) {
            $tabs['checkoutBeta'] = array(
                'label' => $this->l('Checkout v5.0. Try now!'),
                'href' => 'checkoutBeta',
                'icon' => 'shopping-cart',
                'sub_tab' => array(
                    'checkoutBetaGeneral' => array(
                        'label' => $this->l('General'),
                        'href'  => 'checkoutBetaGeneral',
                        'icon'  => 'cogs',
                    ),
                    'checkoutBetaSocialNetwork'  => array(
                        'label' => $this->l('Social Networks'),
                        'href'  => 'checkoutBetaSocialNetwork',
                        'icon'  => 'share-alt',
                    )
                )
            );
    ...
    private function getHelperForm()
    {
        ...
        $checkoutBeta  = $this->getCheckoutBetaForm();
        ...
        $form            = array(
            ...
            'forms' => array(
                ...
                'checkoutBetaGeneral' => $checkoutBeta,
            ),
        );
...
//                && !Tools::getIsset('token')
    ...
    {
        $this->getService('onepagecheckoutps.install.installer')->insertQueriesByShop($params['new_id_shop']);
    }
...
//                        }
    ...
    public function loadPayment()
    {
        ...
        if (Module::isEnabled('minpurchase')) {
            include_once(PS_MODULE_DIR . 'minpurchase/minpurchase.php');
