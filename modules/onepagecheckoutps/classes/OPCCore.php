...
class OPCCore
{
                ...
                if (Tools::getIsset('license_number') && !empty($params['license_number'])) {
                    ...
                    if (is_object($response)) {
                        if ($response->code == $this->CODE_ERROR) {
                            ...
                            Configuration::updateGlobalValue($this->module->prefix_module.'_DOMAIN', $response->domain);
                            Configuration::updateGlobalValue($this->module->prefix_module.'_RM', '1');
    ...
    public function checkModulePTS()
    {
        ...
        if ($this->isModuleActive('languageperdomain')) {
            $new_extension = Db::getInstance()->getRow('
                SELECT *
                FROM `'._DB_PREFIX_.'languageperdomain`
                WHERE `lang_id` = '.(int) $this->context->language->id.'
                AND `target_replace` = '.(int) $this->context->shop->id.'
            ');

            if ($new_extension) {
                $domain_allowed = str_ireplace('www.', '', $new_extension["new_target"]);
                if ($domain_allowed === $server_name) {
                    return true;
                }
            }
        }
        ...
        $tmp_valid = false;
        if (Configuration::get('PS_MULTISHOP_FEATURE_ACTIVE') && !Validate::isLoadedObject($this->context->employee)) {
