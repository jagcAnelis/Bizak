...
trait OnePageCheckoutPSAdapter
{
    ...
    {
        $this->getService('onepagecheckoutps.prestashop.configuration')->fill(
            $this->moduleConfigs
        );
    ...
    {
        $moduleQueries = require_once _PS_MODULE_DIR_ . 'onepagecheckoutps/sql/queries.php';
    ...
    public function getContextProvider()
    {
        if (is_null($this->psContext)) {
            $this->psContext = $this->get('onepagecheckoutps.prestashop.provider.context');
        }
    ...
    public function getShopProvider()
    {
        if (is_null($this->psShop)) {
            $this->psShop = $this->get('onepagecheckoutps.prestashop.provider.shop');
        }
    ...
    public function addSocialNetwork()
    {
        try {
            ...
            $socialNetwork = $this->getService('onepagecheckoutps.core.social_network');
    ...
    public function getTemplateVars()
    {
        ...
        $vars = array();
        $vars[$this->prefix_module] = array(
            'General' => array(
                'Design' => array(
                    'style' => $this->getStyleModule(),
                ),
                'staticToken' => Tools::encrypt('onepagecheckoutps/index'),
            ...
            'Cart' => array(
                ...
                'isEditProductsCartEnabled' => $cart->isEditProductsCartEnabled(),
            ),
        );
    ...
    public function initContentControllerOPCBeta($controller, $pageName = '')
    {
        ...
        $this->getContextProvider()->getSmarty()->assign($this->getTemplateVars());
        Media::addJsDef($this->getTemplateVars(true));
    ...
    public function hookActionUpdateLangAfter($params)
    {
        ...
        if ($sqlLangs) {
            ...
            $shopList = $this->get('onepagecheckoutps.prestashop.provider.shop')->getShops();
            if ($shopList) {
    ...
    {
        $this->getService('onepagecheckoutps.install.installer')->insertQueriesByLang();
