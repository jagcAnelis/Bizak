...
class ShippingService extends AbstractStepCheckout
{
    ...
    public function getDeliveryOptionList()
    {
        ...
        if ($customer->isLogged() || $customer->isGuest()) {
            ...
            $addressesDelivery = $addresses->getCustomerAddressByType('delivery');
        ...
        $deliveryAddress = $addresses->getAddressDeliveryId(true);
        if (!$deliveryAddress) {
            throw new ShippingException(
                'It is necessary to create an delivery address to be able to show the different shipping options.',
                ShippingException::SHIPPING_NEED_ADDRESS
        ...
        $carrierIdSelected = $this->core->getCarrierIdSelected();
    ...
    {
        $addresses = $this->module->get('onepagecheckoutps.core.addresses');
        $idAddress = $addresses->getAddressDeliveryId();

        $vars = array(
            'hookDisplayBeforeCarrier' => Hook::exec(
                'displayBeforeCarrier',
                array('cart' => $this->contextProvider->getCart())
            ),
