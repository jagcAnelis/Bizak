...
class PaymentService extends AbstractStepCheckout
{
    ...
    public function isConfirmAddressDeliveryEnabled()
    {
        if (!$this->contextProvider->isVirtualCart()) {
            if ($this->module->getConfigurationList('OPC_CONFIRM_ADDRESS')) {
                ...
                $addresses = $this->module->get('onepagecheckoutps.core.addresses');
                $addressDelivery = $addresses->getAddressDeliveryId(true);
    ...
    public function getPaymentOptionList()
    {
        ...
        if (!$this->contextProvider->isVirtualCart()) {
            try {
                $deliveryOptionList = $this->module->get('onepagecheckoutps.core.shipping')->getDeliveryOptionList();
    ...
    public function getTemplateVars()
    {
        ...
        if ($this->isShowDiscountBoxOnPaymentEnabled()) {
            $cartService = $this->module->get('onepagecheckoutps.core.cart');

            $vars['cartPresenterVars'] = $cartService->getCartPresenter();
