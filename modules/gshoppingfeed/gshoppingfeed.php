...
class Gshoppingfeed extends Module
{
    ...
    public function __construct()
    {
        ...
        $this->alternativeJs = (bool)Configuration::get('GSHOPPINFEED_ALT_JS');
    ...
    public function getContent()
    {
        ...
        if (Tools::getValue('submitOptionalEdit')) {
            Configuration::updateValue('GSHOPPINFEED_ALT_JS', (bool)Tools::getValue('GSHOPPINFEED_ALT_JS', 0));
        ...
        if ((bool)Tools::isSubmit('update_all_taxonomy_list') === true
            ...
            $id_lang = (int)Tools::getValue('lang_google_lists');
            $updateableLimit = Tools::getValue('updateable_limit', 0);

            if (!$updateableLimit && is_array($item) && count($item) == 2 && is_numeric($item[0])) {
            ...
            if ($updateableLimit && is_array($item) && count($item) == 2 && is_numeric($item[0])) {
                if (!Validate::isInt($updateableLimit) && $updateableLimit <= 0) {
                    $this->errors[] = $this->displayError($this->l('The settings have not been updated! '));
                }

                $rootCat = Configuration::get('PS_ROOT_CATEGORY');
                $catIds = DB::getInstance()->executeS('SELECT id_category FROM ' . _DB_PREFIX_ . 'category
                                                        WHERE id_category NOT IN (SELECT id_category 
                                                            FROM `' . _DB_PREFIX_ . 'gshoppingfeed_taxonomy` 
                                                                WHERE id_lang = ' . (int)$id_lang . ' ) 
                                                        AND id_category != ' . (int)$rootCat . '
                                                        ORDER BY id_category ASC
                                                        LIMIT ' . (int)$updateableLimit);
                if ($catIds && is_array($catIds) && count($catIds)) {
                    $data_insert = array();
                    foreach ($catIds as $catId) {
                        $data_insert[] = array(
                            'id_category' => (int)$catId['id_category'],
                            'id_taxonomy' => (int)$item[0],
                            'name_taxonomy' => pSQL($item[1]),
                            'id_lang' => (int)$id_lang
                        );
        ...
        if (Tools::getValue('rebuild') && Tools::getValue('rebuild') == 1
            ...
            if ($gshoppingfeed && count($gshoppingfeed) > 0) {
                ...
                $ret['google_product_category_rewrite'] = (int)$gshoppingfeed['google_product_category_rewrite'];
                ...
                $ret['shipping_weight_format'] = $gshoppingfeed['shipping_weight_format'];
                ...
                $ret['export_width'] = (int)$gshoppingfeed['export_width'];
                $ret['export_width_inp'] = $gshoppingfeed['export_width_inp'];
                $ret['export_height'] = (int)$gshoppingfeed['export_height'];
                $ret['export_height_inp'] = $gshoppingfeed['export_height_inp'];
                $ret['export_depth'] = (int)$gshoppingfeed['export_depth'];
                $ret['export_depth_inp'] = $gshoppingfeed['export_depth_inp'];
        ...
        return $output . $this->getFastLink() . $this->renderList() . $this->renderLinksTaxonomyList()
            . $this->renderForm() . $this->renderTaxonomyList() . $this->renderOptionalForm() . $this->getVerifyBtn()
    ...
    public function renderOptionalForm()
    {
        ...
        $helper->tpl_vars = array(
            'fields_value' => array(
                'GSHOPPINFEED_ALT_JS' => Configuration::get('GSHOPPINFEED_ALT_JS')
            ),
        ...
        return $helper->generateForm(
            array(
                array(
                    'form' => array(
                        'legend' => array(
                            'title' => $this->l('Optional configure.'),
                            'icon' => 'icon-edit'
                        ),
                        'input' => array(
                            array(
        ...
        $tables = array(
            array(
                ...
                'fields' => array(
                    ...
                    'google_product_category_rewrite' => 'tinyint',
    ...
    public function generationList($param = '', $type_generation = null)
    {
        if ((empty($param) || !isset($param['id_gshoppingfeed'])) && Tools::getValue('submitGshoppingfeedModule') && Tools::getValue('submitGshoppingfeedModule') == 1) {
            ...
            $param['google_product_category_rewrite'] = (Tools::getValue('google_product_category_rewrite') && Tools::getValue('google_product_category_rewrite') == 1) ? 1 : 0;
            ...
            $param['shipping_weight_format'] = (Tools::getValue('shipping_weight_format') && Validate::isInt(Tools::getValue('shipping_weight_format'))) ? (int)Tools::getValue('shipping_weight_format') : 0;
        ...
        $generate_file = md5(_COOKIE_KEY_ . $param['id_gshoppingfeed']) . '.xml';
        ...
        if (isset($param['shipping_weight_format'])) {
            switch ($param['shipping_weight_format']) {
                case 0:
    ...
    protected function getItemXML($product, $param, $id_lang, $image_type, $attribute_flag = false, $attribute_param = array(), $attribute_group = array())
    {
        ...
        $incTax = $this->useTax;
        if (isset($id_country) && $id_country > 0 && $this->useTax) {
            $addr = new Address();
        ...
        if (!empty($_product->weight) && is_numeric($_product->weight) && $_product->weight > 0
            ...
            $weight = round($_product->weight, 5);
            if ($attribute_flag && $attribute_group['id_group'] > 0 && is_numeric($combination_weight) &&
                $combination_weight > 0) {
                $weight += round($combination_weight, 5);
            }
        ...
        if ($param['export_width'] && $_product->width > 0) {
            $xml_entry .= '<g:shipping_width>' . number_format($_product->width, 2, '.', '') . ' ' . $param['export_width_inp'] . '</g:shipping_width>' . "\n";
        }

        if ($param['export_height'] && $_product->height > 0) {
            $xml_entry .= '<g:shipping_height>' . number_format($_product->height, 2, '.', '') . ' ' . $param['export_height_inp'] . '</g:shipping_height>' . "\n";
        }

        if ($param['export_depth'] && $_product->depth > 0) {
            $xml_entry .= '<g:shipping_length>' . number_format($_product->depth, 2, '.', '') . ' ' . $param['export_depth_inp'] . '</g:shipping_length>' . "\n";
        }
    ...
    protected function getExistTemplate()
    {
        ...
        $gshoppingfeed = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('SELECT * FROM `' . _DB_PREFIX_ . 'gshoppingfeed`
            GROUP BY `id_gshoppingfeed` ORDER BY `id_gshoppingfeed` ASC');
        if ($gshoppingfeed && count($gshoppingfeed) > 0) {
            ...
            'form' => array(
                ...
                'input' => array(
                    ...
                    array(
                        ...
                        'label' => $this->l('Width'),
                        'name' => 'export_width',
                        'placeholder' => $this->l('cm or inch'),
                        'desc' => $this->l('Limits: 1 - 400 for cm, 1 - 150 for inch'),
                        'values' => array(
                            array(
                                'id' => 'enabled_on',
                                'value' => 1
                    ...
                    array(
                        'type' => 'switch',
                        'tab' => 'set_other',
                        'label' => $this->l('google_product_category by ID (Yes / No)'),
                        'desc' => $this->l('Using google_product_category ID instead category name'),
    ...
    protected function getConfigFormValues()
    {
        $ret = array(
            ...
            'google_product_category_rewrite' => 0,
    ...
    protected function postProcess()
    {
        if (Tools::getValue('submitGshoppingfeedModule') && Tools::getValue('submitGshoppingfeedModule') == 1) {
            ...
            $insert_data['google_product_category_rewrite'] = (Tools::getValue('google_product_category_rewrite') && Tools::getValue('google_product_category_rewrite') == 1) ? 1 : 0;
            ...
            $insert_data['shipping_weight_format'] = (Tools::getValue('shipping_weight_format') && Validate::isInt(Tools::getValue('shipping_weight_format'))) ? (int)Tools::getValue('shipping_weight_format') : 0;
            ...
            $insert_data['export_width'] = (Tools::getValue('export_width') && Tools::getValue('export_width') == 1) ? 1 : 0;
            $insert_data['export_width_inp'] = pSQL(Tools::getValue('export_width_inp'));
            $insert_data['export_height'] = (Tools::getValue('export_height') && Tools::getValue('export_height') == 1) ? 1 : 0;
            $insert_data['export_height_inp'] = pSQL(Tools::getValue('export_height_inp'));
            $insert_data['export_depth'] = (Tools::getValue('export_depth') && Tools::getValue('export_depth') == 1) ? 1 : 0;
            $insert_data['export_depth_inp'] = pSQL(Tools::getValue('export_depth_inp'));
