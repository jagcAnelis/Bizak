...
class Ttcmsbanner extends Module implements WidgetInterface
{
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->trans('TT - CMS Banner block', array(), 'Modules.CustomText');
    ...
    public function installDB()
    {
        ...
        $return &= Db::getInstance()->execute('
                CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'ttbannerinfo` (
                `id_ttbannerinfo` INT UNSIGNED NOT NULL AUTO_INCREMENT,
                `id_shop` int(10) unsigned DEFAULT NULL,
                PRIMARY KEY (`id_ttbannerinfo`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=utf8 ;');

        $return &= Db::getInstance()->execute('
                CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'ttbannerinfo_lang` (
                `id_ttbannerinfo` INT UNSIGNED NOT NULL,
                `id_lang` int(10) unsigned NOT NULL ,
                `text` text NOT NULL,
                PRIMARY KEY (`id_ttbannerinfo`, `id_lang`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=utf8 ;');
    ...
    public function uninstallDB($drop_table = true)
    {
        $ret = true;
        if ($drop_table) {
            $ret &=  Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'ttbannerinfo`') && Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'ttbannerinfo_lang`');
        }
    ...
    {
        $info = new ttcmsbannerinfo(Tools::getValue('id_ttbannerinfo', 1));
    ...
    protected function renderForm()
    {
        ...
        if (Shop::isFeatureActive() && Tools::getValue('id_ttbannerinfo') == false) {
    ...
    public function getFormValues()
    {
        ...
        foreach (Language::getLanguages(false) as $lang) {
            $info = new ttcmsbannerinfo((int)$id_info);
    ...
    {
        $sql = 'SELECT r.`id_ttbannerinfo`, r.`id_shop`, rl.`text`
            FROM `'._DB_PREFIX_.'ttbannerinfo` r
            LEFT JOIN `'._DB_PREFIX_.'ttbannerinfo_lang` rl ON (r.`id_ttbannerinfo` = rl.`id_ttbannerinfo`)
