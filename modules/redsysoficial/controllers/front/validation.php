...
class RedsysoficialValidationModuleFrontController extends ModuleFrontController  {
    public function postProcess() {
        try{
            ...
            if ($accesoDesde === 'POST' || $accesoDesde === 'GET') {
                ...
                escribirLog($idLog." -- "."Pedido de Redsys: ".$pedido,$logActivo);
                    ...
                    && checkRespuesta($respuesta)) {
                        if ($accesoDesde === 'POST') {
                            ...
                            if (!$carrito_valido){
                                ...
                                $redsys->validateOrder($pedido, _PS_OS_ERROR_, $total/100, $redsys->displayName, $mensajeError);
                            ...
                            if (!$cliente)
                                ...
                                if ($monedaOrig == $moneda && (int)$codigoOrig == (int)$codigo && $respuesta < 101 && checkAutCode($id_trans)) {
                                    /** Compra válida **/
                                    $mailvars['transaction_id'] = (int)$id_trans;
                                    $redsys->validateOrder($cart->id, Configuration::get("REDSYS_ESTADO_PEDIDO"), $total/100, $redsys->displayName, null, $mailvars, (int)$cart->id_currency, false, (property_exists($customer, "secure_key") && !is_null($customer->secure_key)) ? $customer->secure_key : false);
