...
include(dirname(__FILE__).'/redsysoficial.php');
		...
		if ($firma_local === $firma_remota
			...
			if ($accesoDesde === 'POST') {
				...
				if ($monedaOrig == $moneda && (int)$codigoOrig == (int)$codigo && $respuesta < 101 && checkAutCode($id_trans)) {
					...
					$redsys->validateOrder($pedido, Configuration::get("REDSYS_ESTADO_PEDIDO"), $total/100, $redsys->displayName, null, $mailvars, (int)$cart->id_currency, false, $customer->secure_key);
